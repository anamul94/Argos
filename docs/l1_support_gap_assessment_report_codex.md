...
Estimated current alignment: **25-35%** against the 7 high-level client requirements.
...
### F2. SQS-wrapped payload handling inconsistency (Critical)
- Event detection supports SQS wrapper, but later classification/triage logic reads top-level payload fields directly.
- This can produce bad dedup keys, wrong classification context, or failed investigations.

Evidence:
- `main.py` (`_is_cloudwatch_alarm_event`, `_run_triage`)
- `agents/alert_triage/nodes/classify.py` (reads `state["raw_payload"]["detail"]`)

Impact:
- Missed or misprocessed real alerts.

---

### F3. Dedup flapping suppression after failed triage (High)
- Dedup flap key is written before graph execution.
- If graph fails, cooldown may suppress follow-up real alerts.

Evidence:
- `main.py` (`mark_processing`, exception handling)
- `utils/deduplication.py`

Impact:
- Possible alert blindness during failure windows.

---

### F4. Alert-state filtering is incomplete at app layer (High)
- App checks only event source/type, not detail state=ALARM.

Evidence:
- `main.py` (`_is_cloudwatch_alarm_event`)

Impact:
- Higher noise and unnecessary triage runs.
...
2. Normalize inbound event format once (direct + SQS) before dedup/classify.
3. Ensure ALARM-state filtering before invoking triage.
4. Fix dedup failure semantics to avoid suppressing valid post-failure alerts.
5. Remove payload `print()` and tighten sensitive logging controls.
